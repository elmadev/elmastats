...
<?php
  ...
  if ($_GET["country"] == "") {
    ...
    foreach ($users as $cItem) {
      if ($cItem["country"] != "") {
        $duplicate = false;
        foreach ($nc as $n) if ($cItem["country"] == $n) $duplicate = true;
        if (!$duplicate) {
          $nc[] = $cItem["country"];
          $ncc[$cItem["country"]] = 1;
        } else {
    ...
    foreach ($nc as $n) {
      ...
      echo(flag($n) . " <a href=\"natl.php?country=" . $n . "\">" . $cnames[strtoupper($n)] . "</a><br/>");
      $wry = "s"; if ($ncc[$n] == 1) $wry = "";
      echo("</td><td>" . $ncc[$n] . " kuski" . $wry . "</td></tr>");
    }
    echo("</table>");
    $eror = true;
  }
  ...
  if (!$eror) {
    ...
    for ($y = 0;$y < count($tts);$y++) {
      ...
      echo("<td class=\"times\" width=\"81px\"" . $str . "><center>" . formatElmaTime($tts[$y]["time"]) . "</center></td>");
    ...
    for ($x = 0;$x < 54;$x++) {
      ...
      echo("<th class=\"times\" width=\"180px\">" . $intnames[$x+1] . "</th>");
      ...
      for ($y = 0;$y < $topnum;$y++) {
        if ($top[$y]["nick"] != "") {
          ...
          echo("<td class=\"times\" width=\"81px\"" . $str . "><center>" . sttime($top[$y]["nick"], $x+1, $top[$y]["time"]) . "</center></td>");
          echo("</tr>");
        }
      }
