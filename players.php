<?php include("top.php"); ?>
<?php
  echo("<b>Players (" . count($users) . ")</b><br/><br/>");
  echo("<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">");
  $i = 0;
  $rowsize = 5; //how many palyers per row
  $type = 1; //0: klassik    1: detailed
  if ($type == 1) {
  ...
  foreach ($users as $cItem) {
    ...
    if ($cItem["nick"] != "") {
      $f = true;
      if ($_GET["name"] != "") {
        if (strpos(strtolower($cItem["nick"]), strtolower($_GET["name"])) === false) $f = false;
      }
      if ($_GET["country"] != "") {
        if (strpos(strtolower($cItem["country"]), strtolower($_GET["country"])) === false) $f = false;
      }
      ...
      if ($f) {
        switch ($type) {
          case 0:
            ...
            break;
          case 1:
            echo("<tr><td>" . man($cItem["nick"]) . "</td>");
            echo("<td>" . date("d/m/y - H:i:s", ttime($cItem["registered"])) . "</td></tr>");
            break;
        }
      }
    }
